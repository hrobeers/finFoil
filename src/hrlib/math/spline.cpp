...
//  Parameters:
//
//    Input, qreal TDATA(5), the nodes associated with the basis function.
//    The entries of TDATA are assumed to be distinct and increasing.
//
//    Input, qreal TVAL, a point at which the B spline basis function is
//    to be evaluated.
//
//    Output, qreal BASIS_FUNCTION_B_VAL, the value of the function at TVAL.
//
{
# define NDATA 5
...
//
  ...
  {
      cerr << "\n";
      cerr << "BASIS_FUNCTION_B_VAL - Fatal error!\n";
      cerr << "  yval not initialised";
      exit ( 1 );
  }
...
//  Parameters:
//
//    Input, qreal BETA1, the skew or bias parameter.
//    BETA1 = 1 for no skew or bias.
//
//    Input, qreal BETA2, the tension parameter.
...
//    to be evaluated.
//
//    Output, qreal BASIS_FUNCTION_BETA_VAL, the value of the function at TVAL.
//
{
# define NDATA 5
...
//
  ...
  {
      cerr << "\n";
      cerr << "BASIS_FUNCTION_BETA_VAL - Fatal error!\n";
      cerr << "  yval not initialised";
      exit ( 1 );
  }
...
//  Parameters:
//
//    Output, qreal BASIS_MATRIX_B_UNI[4*4], the basis matrix.
//
{
  int i;
  int j;
  qreal *mbasis;
  qreal mbasis_save[4*4] = {
    -1.0 / 6.0,
     3.0 / 6.0,
    -3.0 / 6.0,
     1.0 / 6.0,
...
//    BETA2 = 0 for no tension.
//
//    Output, qreal BASIS_MATRIX_BETA_UNI[4*4], the basis matrix.
//
{
  qreal delta;
  int i;
...
//  Parameters:
//
//    Output, qreal BASIS_MATRIX_BEZIER[4*4], the basis matrix.
//
{
  qreal *mbasis;
...
//  Parameters:
//
//    Output, qreal BASIS_MATRIX_HERMITE[4*4], the basis matrix.
//
{
  qreal *mbasis;
...
//  Parameters:
//
//    Input, qreal ALPHA, BETA.
//    ALPHA = || P2 - P1 || / ( || P3 - P2 || + || P2 - P1 || )
//    BETA  = || P3 - P2 || / ( || P4 - P3 || + || P3 - P2 || ).
//
//    Output, qreal BASIS_MATRIX_OVERHAUSER_NONUNI[4*4], the basis matrix.
//
{
  qreal *mbasis;
...
//    ALPHA = || P2 - P1 || / ( || P3 - P2 || + || P2 - P1 || )
//
//    Output, qreal BASIS_MATRIX_OVERHAUSER_NUL[3*3], the basis matrix.
//
{
  qreal *mbasis;

  mbasis = new qreal[3*3];
...
//
//    Output, qreal BASIS_MATRIX_OVERHAUSER_NUR[3*3], the basis matrix.
//
{
  qreal *mbasis;
...
//  Parameters:
//
//    Output, qreal BASIS_MATRIX_OVERHASUER_UNI[4*4], the basis matrix.
//
{
  qreal *mbasis;
...
//  Parameters:
//
//    Output, qreal BASIS_MATRIX_OVERHASUER_UNI_L[3*3], the basis matrix.
//
{
  qreal *mbasis;
...
//  Parameters:
//
//    Output, qreal BASIS_MATRIX_OVERHASUER_UNI_R[3*3], the basis matrix.
//
{
  qreal *mbasis;
...
//
//    Input, qreal MBASIS[N*N], the basis matrix.
//
//    Input, int NDATA, the dimension of the vectors TDATA and YDATA.
//
//    Input, qreal TDATA[NDATA], the abscissa values.  This routine
//    assumes that the TDATA values are uniformly spaced, with an
//    increment of 1.0.
//
//    Input, qreal YDATA[NDATA], the data values to be interpolated or
//    approximated.
//
//    Input, qreal TVAL, the value of T at which the spline is to be
//    evaluated.
//
//    Output, qreal BASIS_MATRIX_TMP, the value of the spline at TVAL.
//
{
  qreal arg;
  int first;
  ...
  {
      cerr << "\n";
      cerr << "BASIS_MATRIX_TMP - Fatal error!\n";
      cerr << "  first & arg not initialised";
      exit ( 1 );
  }
//
//  TVEC(I) = ARG**(N-I).
...
//    must be at least 0.
//
//    Input, qreal T, the point at which the Bezier curve should
//    be evaluated.  The best results are obtained within the interval
//    [0,1] but T may be anywhere.
//
//    Input, qreal XCON[0:N], YCON[0:N], the X and Y coordinates
...
//
//    Output, qreal *XVAL, *YVAL, the X and Y coordinates of the point
...
//    must be at least 0.
//
//    Input, qreal X, the point at which the Bezier function should
//    be evaluated.  The best results are obtained within the interval
//    [A,B] but X may be anywhere.
//
//    Input, qreal A, B, the interval over which the Bezier function
...
//
//    Input, qreal Y[0:N], a set of data defining the Y coordinates
//    of the control points.
//
//    Output, qreal BEZ_VAL, the value of the Bezier function at X.
//
{
  qreal *bval;
  int i;
...
//
//    Input, qreal A, B, the endpoints of the interval on which the
//    approximant is based.  A and B should not be equal.
//
//    Input, qreal YDATA[0:N], the data values at N+1 equally spaced points
//    in [A,B].  If N = 0, then the evaluation point should be 0.5 * ( A + B).
//    Otherwise, evaluation point I should be ( (N-I)*A + I*B ) / N ).
//
//    Input, qreal XVAL, the point at which the Bernstein polynomial
//    approximant is to be evaluated.  XVAL does not have to lie in the
//    interval [A,B].
//
//    Output, qreal BPAB_APPROX, the value of the Bernstein polynomial approximant
...
//
//    Input, qreal X, the evaluation point.
//
//    Output, qreal BP01[0:N], the values of the N+1 Bernstein basis
//    polynomials at X.
//
{
  qreal *bern;
...
//
//    Input, qreal X, the point at which the polynomials are to be
//    evaluated.  X need not lie in the interval [A,B].
//
//    Output, qreal BERN[0:N], the values of the N+1 Bernstein basis
//    polynomials at X.
//
{
  qreal *bern;
...
//  Parameters:
//
//    Input, qreal X1, X2, the endpoints of the interval of
//    definition of the cubic.  X1 and X2 must be distinct.
//
//    Input, qreal F1, F2, the values of the function at X1 and
//    X2, respectively.
//
//    Input, qreal D1, D2, the derivative values at X1 and
//    X2, respectively.
//
//    Input, int NE, the number of evaluation points.
//
//    Input, qreal XE[NE], the points at which the function is to
//    be evaluated.  If any of the XE are outside the interval
//    [X1,X2], a warning error is returned in NEXT.
//
//    Output, qreal FE[NE], the value of the cubic function
//    at the points XE.
//
...
//
//    Input, qreal A[3*N], the D3 matrix.
//
//    Input, qreal X[N], the vector to be multiplied by A.
//
//    Output, qreal D3_MXV[N], the product A * x.
//
{
  qreal *b;
  int i;
...
//    Input, int N, the order of the linear system.
//
//    Input/output, qreal A[3*N].
//    On input, the nonzero diagonals of the linear system.
//    On output, the data in these vectors has been overwritten
//    by factorization information.
//
//    Input, qreal B[N], the right hand side.
//
//    Output, qreal D3_NP_FS[N], the solution of the linear system.
...
void d3_print ( int n, qreal a[], string title )
...
//
//    Output, qreal D3_UNIFORM[3*N], the D3 matrix.
//
{
  qreal *a;
  int i;
...
//
//    Input, qreal XTAB[NTAB], the X values at which data was taken.
//    These values must be distinct.
//
//    Input, qreal YTAB[NTAB], the corresponding Y values.
//
//    Output, qreal DIFTAB[NTAB], the divided difference coefficients
...
//
//    Input, qreal XTAB[NTAB], the X values upon which the
//    divided difference polynomial is based.
//
//    Input, qreal DIFTAB[NTAB], the divided difference table.
//
//    Input, qreal XVAL, a value of X at which the polynomial
//    is to be evaluated.
//
//    Output, qreal DIF_VAL, the value of the polynomial at XVAL.
//
{
  int i;
  qreal value;
...
//
//    Input, qreal X[POINT_NUM], the abscissas of the data points.
//    At least NTERMS of the values in X must be distinct.
//
//    Input, qreal F[POINT_NUM], the data values at the points X(*).
//
//    Input, qreal W[POINT_NUM], the weights associated with
...
//
//    Output, qreal B[NTERMS], C[NTERMS], D[NTERMS], are quantities
...
//
  pjm1 = new qreal[point_num];
  pj = new qreal[point_num];
...
//
//    Input, qreal B[NTERMS], C[NTERMS], D[NTERMS], the information
//    computed by R8POLY_LS_SET.
//
//    Input, qreal X, the point at which the least squares polynomial
//    is to be evaluated.
//
//    Output, qreal LEAST_VAL, the value of the least squares
//    polynomial at X.
//
{
  int i;
...
//    is to be evaluated.
//
//    Output, qreal *PX, *PXP, the value and derivative of the least
...
//
//    Input, qreal XTAB[NTAB], the X data.  The values in XTAB
//    should be distinct, and in increasing order.
//
//    Input, qreal YTAB[NTAB], the Y data values corresponding
...
//    equal to NTAB-1.
//
//    Output, qreal PTAB[NTAB], the value of the least squares polynomial
//    at the points XTAB(1:NTAB).
//
//    Output, qreal B[1:NDEG], C[0:NDEG], D[2:NDEG], arrays containing
//    data about the polynomial.
//
//    Output, qreal *EPS, the root-mean-square discrepancy of the
//    polynomial fit.
//
...
//
  ...
  rn0 = ntab;
  c[0] = y_sum / ( qreal ) ( ntab );

  for ( i = 0; i < ntab; i++ )
  {
    ptab[i] = y_sum / ( qreal ) ( ntab );
  }
...
//
  ...
  b[1+B_OFFSET] = ztab[0] / ( qreal ) ( ntab );
...
//  Parameters:
//
//    Input, qreal X, the point at which the polynomial is to be evaluated.
//
//    Input, int NDEG, the degree of the polynomial fit used.
//    This is the value of NDEG as returned from LEAST_SET_OLD.
//
//    Input, qreal B[1:NDEG], C[0:NDEG], D[2:NDEG], arrays defined by
//    LEAST_SET, and needed to evaluate the polynomial.
//
//    Output, qreal LEAST_VALPOLD, the value of the polynomial at X.
//
{
  int B_OFFSET = -1;
  int D_OFFSET = -2;
...
//
//    Input, qreal TDATA[NDATA], the abscissas of the data points.  The
//    values in TDATA must be in strictly ascending order.
//
//    Input, qreal YDATA[NDIM*NDATA], the data points corresponding to
//    the abscissas.
//
...
//
//    Input, qreal TVAL, the value of T at which the parabolic interpolant
//    is to be evaluated.  Normally, TDATA[0] <= TVAL <= T[NDATA-1], and
//    the data will be interpolated.  For TVAL outside this range,
//    extrapolation will be used.
//
//    Output, qreal YVAL[NDIM], the value of the parabolic interpolant
//    at TVAL.
//
{
  qreal dif1;
...
//  Parameters:
//
//    Input, qreal ARG1, ARG2, two values to check.
//
//    Output, qreal PCHST,
...
//
{
  ...
  else if ( arg1 < 0.0 )
  {
    ...
    {
        cerr << "\n";
        cerr << "PCHST - Fatal error!\n";
        cerr << "  value not initialised";
        exit ( 1 );
    }
  }
  else if ( 0.0 < arg1 )
...
//
//    Output, qreal R8_UNIFORM_01, a new pseudorandom variate, strictly between
//    0 and 1.
//
{
  int k;
...
//
//    Input, qreal T[N], an array that has been sorted into ascending order.
//
//    Input, qreal TVAL, a value to be bracketed by entries of T.
//
...
//
//    Input, qreal ALO, AHI, the low and high values.
//
//    Output, qreal R8VEC_EVEN_NEW[N], N evenly spaced values.
...
//
//    Output, qreal R8VEC_INDICATOR_NEW[N], the indicator array.
//
{
  qreal *a;
  int i;
...
//
//    Input, qreal X[N], the array to be checked.
//
...
void r8vec_print ( int n, qreal a[], string title )
...
//
//    Input, qreal A[N], the vector to be printed.
//
...
//
//    Input, qreal B, C, the lower and upper limits of the pseudorandom values.
//
//    Input/output, int *SEED, a seed for the random number generator.
//
//    Output, qreal R8VEC_UNIFORM_NEW[N], the vector of pseudorandom values.
//
{
  int i;
  ...
  for ( i = 0; i < n; i++ )
  {
    ...
    *seed = 16807 * ( *seed - k * 127773 ) - k * 2836;
    ...
    r[i] = b + ( c - b ) * ( qreal ) ( *seed ) * 4.656612875E-10;
  }
...
//    Input, int N, the number of elements of A.
//
//    Input, qreal A[N], the array to examine, which does NOT have to
//    be sorted.
//
//    Input, qreal TOL, a tolerance for checking equality.
//
...
//    Input, int N, the number of entries in the vector.
//
//    Output, qreal A[N], a vector of zeroes.
//
{
  int i;
...
//
//    Input, qreal TDATA[NDATA], the abscissas of the data.
//
//    Input, qreal YDATA[NDATA], the data values.
//
//    Input, qreal TVAL, a point at which the spline is to be evaluated.
//
...
//    Input, qreal TVAL, a point at which the spline is to be evaluated.
//
//    Output, qreal SPLINE_BETA_VAL, the value of the function at TVAL.
//
{
  qreal a;
  qreal b;
...
qreal spline_constant_val ( int ndata, qreal tdata[], qreal ydata[],
  qreal tval )
...
//
//    Input, qreal TDATA[NDATA-1], the breakpoints.  The values of TDATA should
//    be distinct and increasing.
//
//    Input, qreal YDATA[NDATA], the values of the spline in the intervals
//    defined by the breakpoints.
//
//    Input, qreal TVAL, the point at which the spline is to be evaluated.
//
//    Output, qreal *SPLINE_CONSTANT_VAL, the value of the spline at TVAL.
//
{
  int i;
...
qreal *spline_cubic_set ( int n, qreal t[], qreal y[], int ibcbeg,
  qreal ybcbeg, int ibcend, qreal ybcend )
...
//
//    Input, qreal T[N], the knot values, that is, the points were data is
//    specified.  The knot values should be distinct, and increasing.
//
//    Input, qreal Y[N], the data values to be interpolated.
//
...
//
//    Input, qreal YBCBEG, the values to be used in the boundary
...
//
//    Input, qreal YBCEND, the values to be used in the boundary
//    conditions if IBCEND is equal to 1 or 2.
//
//    Output, qreal SPLINE_CUBIC_SET[N], the second derivatives of the cubic spline.
//
{
  qreal *a;
  qreal *b;
...
//
//    Input, qreal T[N], the knot values.
//
//    Input, qreal Y[N], the data values at the knots.
//
//    Input, qreal YPP[N], the second derivatives of the spline at
//    the knots.
//
//    Input, qreal TVAL, a point, typically between T[0] and T[N-1], at
//    which the spline is to be evalulated.  If TVAL lies outside
//    this range, extrapolation is used.
//
//    Output, qreal *YPVAL, the derivative of the spline at TVAL.
//
//    Output, qreal *YPPVAL, the second derivative of the spline at TVAL.
//
//    Output, qreal SPLINE_VAL, the value of the spline at TVAL.
//
{
  qreal dt;
  qreal h;
...
//    the knots.
//
//    Output, qreal *YVAL, *YPVAL, *YPPVAL, the value of the spline, and
...
//
//    Input, qreal TDATA[NDATA], the abscissas of the data points.
//    The entries of TDATA are assumed to be strictly increasing.
//
//    Input, qreal Y[NDATA], YP[NDATA], the value of the
//    function and its derivative at TDATA(1:NDATA).
//
//    Output, qreal SPLINE_HERMITE_SET[4*NDATA], the coefficients of
...
//    The entries of TDATA are assumed to be strictly increasing.
//
//    Input, qreal C[4*NDATA], the coefficient data computed by
//    SPLINE_HERMITE_SET.
//
//    Input, qreal TVAL, the point where the interpolant is to
//    be evaluated.
//
//    Output, qreal *SVAL, *SPVAL, the value of the interpolant
...
qreal spline_linear_int ( int ndata, qreal tdata[], qreal ydata[],
  qreal a, qreal b )
...
//    Input, int NDATA, the number of data points defining the spline.
//
//    Input, qreal TDATA[NDATA], YDATA[NDATA], the values of the independent
//    and dependent variables at the data points.  The values of TDATA should
//    be distinct and increasing.
//
//    Input, qreal A, B, the interval over which the integral is desired.
//
//    Output, qreal SPLINE_LINEAR_INT, the value of the integral.
//
{
  qreal a_copy;
  int a_left;
...
void spline_linear_intset ( int n, qreal int_x[], qreal int_v[],
...
//
//    Input, qreal INT_X[N+1], the points that define the intervals.
//    Interval I lies between INT_X(I) and INT_X(I+1).
//
//    Input, qreal INT_V[N], the desired value of the integral of the
//    linear spline over each interval.
//
//    Output, qreal DATA_X[N], DATA_Y[N], the values of the independent
...
//    Input, qreal TVAL, the point at which the spline is to be evaluated.
//
//    Output, qreal *YVAL, *YPVAL, the value of the spline and its first
...
//    Input, qreal YDATA[NDATA], the data values.
//
//    Input, qreal TVAL, the value where the spline is to
//    be evaluated.
//
//    Output, qreal SPLINE_OVERHAUSER_NONUNI_VAL, the value of the
//    spline at TVAL.
//
{
  qreal alpha;
...
//    be evaluated.
//
//    Output, qreal SPLINE_OVERHAUSER_UNI_VAL, the value of the spline at TVAL.
//
{
  int left;
  qreal *mbasis;
...
//
  ...
  else
  {
      cerr << "\n";
      cerr << "SPLINE_OVERHAUSER_UNI_VAL - Fatal error!\n";
      cerr << "  mbasis & yval not initialised";
      exit ( 1 );
  }
...
void spline_overhauser_val ( int ndim, int ndata, qreal tdata[],
  qreal ydata[], qreal tval, qreal yval[] )
...
//    the abscissas.
//
//    Input, qreal TVAL, the abscissa value at which the spline
//    is to be evaluated.  Normally, TDATA[0] <= TVAL <= T[NDATA-1], and
//    the data will be interpolated.  For TVAL outside this range,
//    extrapolation will be used.
//
//    Output, qreal YVAL[NDIM], the value of the spline at TVAL.
//
{
  int i;
  int left;
...
//    Input, int N, the number of data points.  N must be at least 2.
//
//    Input, qreal X[N], the strictly increasing independent
//    variable values.
//
//    Input, qreal F[N], dependent variable values to be interpolated.  This
...
//
//    Output, qreal D[N], the derivative values at the
...
void spline_pchip_val ( int n, qreal x[], qreal f[], qreal d[],
  int ne, qreal xe[], qreal fe[] )
...
//    variable values.
//
//    Input, qreal F[N], the function values.
//
//    Input, qreal D[N], the derivative values.
//
//    Input, int NE, the number of evaluation points.
//
//    Input, qreal XE[NE], points at which the function is to
//    be evaluated.
//
//    Output, qreal FE[NE], the values of the cubic Hermite
//    function at XE.
//
{
  int i;
